...
class BBRSIOptimizedStrategy(IStrategy):
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                (dataframe['close'] < dataframe['bb_lowerband_1sd']) # Signal price is less than lower bb
            ),
            'buy'] = 1
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (
                    (dataframe['rsi'] > 93) &
                    (dataframe['close'] < dataframe['bb_upperband_1sd'])  # Signal price is less than mid bb
            ),
            'sell'] = 1
